...
public:
	...
	DnaSequence& operator+(const DnaSequence& rhs) const;
	DnaSequence& operator+=(const DnaSequence& dna);
...
inline DnaSequence& DnaSequence::operator+(const DnaSequence& rhs) const
{
	std::string s = get() + rhs.get();
	DnaSequence* res = new DnaSequence(s);
	return *res;
}
